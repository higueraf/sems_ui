...
/** Devuelve la URL de embed de YouTube */
...
function EventPanel({ event }: { event: ScientificEvent }) {
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row gap-6 items-start">
        ...
        <div className="flex-1">
          <div className="flex flex-wrap gap-2 mb-2">
            {event.edition && (
              <span className="badge bg-primary-100 text-primary-700 text-xs font-bold">
                {event.edition}
              </span>
            )}
          ...
          {event.tagline && (
            <p className="text-primary-600 text-base mt-1 italic">{event.tagline}</p>
          )}
      ...
      {event.videos && event.videos.length > 0 ? (
        ...
        <div className="rounded-xl border border-dashed border-gray-200 bg-gray-50 p-8 text-center text-gray-400">
          ...
          <p className="text-sm">Próximamente se agregarán los videos de este simposio.</p>
        </div>
      )}
    </div>
  );
      ...
      <div className={`py-16 relative overflow-hidden ${isDark ? 'bg-gray-900' : 'bg-primary-700'}`}>
        ...
        <div className="relative max-w-7xl mx-auto px-4 text-center">
          <span className={`text-[11px] font-bold uppercase tracking-[0.2em] block mb-3 ${isDark ? 'text-primary-400' : 'text-primary-200'}`}>
            Simposio Internacional de Ciencia Abierta
          </span>
          ...
          <p className={`max-w-2xl mx-auto text-base ${isDark ? 'text-gray-400' : 'text-primary-100'}`}>
            Revive los mejores momentos de nuestros simposios pasados. Encuentra ponencias,
            conferencias y recursos de cada edición.
          </p>
        </div>
      </div>
        ...
        {!isLoading && (!events || events.length === 0) && (
          <div className="text-center py-20">
            ...
            <h2 className="font-heading font-bold text-xl text-gray-500 mb-2">
              Aún no hay simposios anteriores
            </h2>
            <p className="text-gray-400 text-sm">
              Los eventos pasados aparecerán aquí una vez finalicen.
            </p>
          </div>
        )}
          ...
          <div className="space-y-8">
            ...
            {events.length > 1 && (
              <div className="relative">
                ...
                {/* Indicadores de puntos (cuando hay muchos) */}
                {events.length > 3 && (
            ...
            <div className={`${card} rounded-2xl shadow-sm border p-6 md:p-8`}>
              <EventPanel event={events[selected]} />
            </div>

            {/* ── Navegación inferior (solo si hay más de un evento) ── */}
            {events.length > 1 && (
