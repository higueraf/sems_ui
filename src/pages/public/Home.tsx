    ...
    <div className={`font-body ${bg} ${text}`}>
      ...
      <section className={`relative min-h-[92vh] flex flex-col justify-center overflow-hidden ${isDark ? 'bg-gray-950' : 'bg-gray-50'}`}>
        ...
        <div
          ...
          style={{
            // hero-bg.jpg = imagen del simposio sin texto (public/images/hero-bg.jpg)
        ...
        <div className={`absolute inset-0 bg-gradient-to-tr ${
          isDark ? 'from-primary-950/50 via-transparent to-transparent'
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 w-full">
          <div className="max-w-2xl">
            ...
            {/* Location badge */}
            <div className={`inline-flex items-center gap-2 border text-[11px] font-bold px-4 py-2 rounded-full mb-8 uppercase tracking-widest ${
              isDark
            ...
            <div className="flex items-stretch w-fit mb-5">
              <div className={`border-2 px-6 py-2 font-bold tracking-[0.15em] text-lg ${
                isDark ? 'border-white/55 text-white' : 'border-primary-900 text-primary-900'
              }`}>
                18–22 DE MAYO
              </div>
              <div className={`px-5 py-2 font-black text-lg tracking-widest ${
                isDark ? 'bg-[#00ACC1] text-white' : 'bg-primary-700 text-white'
              }`}>
                2026
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-4 mb-14">
              <Link
                to="/postular"
                className={`inline-flex items-center gap-2 font-bold px-8 py-3.5 rounded-xl transition-all text-base text-white ${
                  isDark
              ...
              <Link
                to="/pautas"
                className={`inline-flex items-center gap-2 border-2 font-semibold px-8 py-3.5 rounded-xl transition-all text-base ${
                  isDark
              ...
              <div className="flex gap-3">
                {[
                  ...
                  <div key={l} className="text-center">
                    <div className={`border rounded-xl w-16 h-16 md:w-[74px] md:h-[74px] flex items-center justify-center backdrop-blur-sm ${
                      isDark
        ...
        <div className={`relative border-t ${
          isDark ? 'border-white/8 bg-black/25 backdrop-blur-sm' : 'border-primary-100 bg-primary-700'
        }`}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 divide-x divide-y md:divide-y-0 divide-gray-100 dark:divide-white/10">
            {QUICK_LINKS.map((link) => (
              <Link
                key={link.to}
                to={link.to}
                className={`flex items-center justify-between gap-4 px-6 py-5 group transition-all ${
                  isDark ? 'hover:bg-primary-900/30' : 'hover:bg-[#F5EDD8]'
                }`}
              >
            ...
            <div className="grid lg:grid-cols-2 gap-16 items-center">
                ...
                {(aboutSection.metadata?.slogans?.length ?? 0) > 0 && (
                  <div className="mt-6 space-y-2">
                    {aboutSection.metadata?.slogans?.map((s: string, i: number) => (
                      <div key={i} className={`flex items-start gap-3 px-4 py-3 rounded-lg border-l-4 ${
                        isDark ? 'bg-primary-900/25 border-primary-500 text-primary-300' : 'bg-primary-50 border-primary-600 text-primary-800'
                      }`}>
              ...
              <div className="grid grid-cols-2 gap-5">
                {[
                  ...
                  <div key={stat.label} className={`rounded-2xl p-6 text-center shadow-sm transition-transform hover:-translate-y-1 ${
                    isDark
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {event.thematicAxes.map((axis, i) => (
                <div key={axis.id} className={`group rounded-2xl p-6 border transition-all duration-300 hover:shadow-lg animate-fade-in-up ${
                  isDark ? 'bg-gray-900 border-white/8 hover:border-primary-500/35' : 'bg-white border-gray-100 hover:border-primary-200'
          ...
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="space-y-3">
              {datesSection.metadata.dates.map((item: any, i: number) => (
                <div key={i} className={`flex items-center gap-5 p-5 rounded-2xl border-l-4 transition-all hover:translate-x-1.5 ${
                  item.highlight
                  ...
                  <div className={`flex-shrink-0 text-sm font-black px-4 py-1.5 rounded-lg ${
                    item.highlight
